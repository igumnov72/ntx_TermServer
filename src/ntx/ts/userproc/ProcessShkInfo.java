...
import ntx.sap.fm.Z_TS_SHK_INFO;
 ...
 * @author amolchanov
 */
public class ProcessShkInfo extends ProcessTask {
    
  private final ShkInfoData d = new ShkInfoData();

  public ProcessShkInfo(long procId) throws Exception {
    super(ProcType.SHKINFO, procId);
  }
  ...
  public FileData handleQuery(TermQuery tq, UserContext ctx) throws Exception {
    ...
    if (getTaskState() == TaskState.START && scan == null && menu == null) {
      ...
      return htmlWork("ШК Инфо", false, ctx);
  }
...
// Проверка стеллажа
          Z_TS_SHK_INFO ff = new Z_TS_SHK_INFO();
          ff.SHK = scan;
          ff.execute();
          if (ff.isErr) {  
            callSetErr(ff.err, ctx);
             ...
             if (ff.INF9 != "") {
             callAddHist(ff.INF12, ctx);
             callAddHist(ff.INF11, ctx);
             callAddHist(ff.INF10, ctx);
             callAddHist(ff.INF9, ctx);
          }             
             
             callAddHist(ff.INF8, ctx);
             callAddHist(ff.INF7, ctx);
             callAddHist(ff.INF6, ctx);
             callAddHist(ff.INF5, ctx);
             callAddHist(ff.INF4, ctx);
             callAddHist(ff.INF3, ctx);
             callAddHist(ff.INF2, ctx);
             callAddHist(ff.INF1, ctx);

//             callSetMsg(ff.INFO, ctx);                

             callSetTaskState(TaskState.SEL_SHK, ctx);
          
//          d.callSetStell(scan, ff.INFO, TaskState.SEL_CHARG, this, ctx);  
          }        
//          return htmlWork("ШК Инфо", true, ctx);
        }
    }
...
*/
    HtmlPageMenu p = new HtmlPageMenu("Меню", "ШК Инфо",
...
//AM 12.01.2024
class ShkInfoData extends ProcData {
