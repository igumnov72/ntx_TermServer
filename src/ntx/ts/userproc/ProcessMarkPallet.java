...
import ntx.sap.fm.Z_TS_MARK_PALLET_SHK;
import ntx.sap.fm.Z_TS_MARK_PALLET_SHKLIST_PRINT;
 ...
 * @author amolchanov
 */
public class ProcessMarkPallet extends ProcessTask {
    
  private final MarkPalletData d = new MarkPalletData();

  public ProcessMarkPallet(long procId) throws Exception {
    super(ProcType.MARK_PALLET, procId);
  }
  ...
  public FileData handleQuery(TermQuery tq, UserContext ctx) throws Exception {
    ...
    if (getTaskState() == TaskState.START && scan == null && menu == null) {
      ...
      return htmlWork("Маркировка паллет", false, ctx);
  }
...
// AM 10.06.2024
    if (d.scanIsDouble(scan)) {
      ...
      return htmlWork("Маркировка паллет", true, ctx);
    }
...
//          callSetMsg(sInfo + "  " + w1_qty.toString() + "[" + Integer.toString(d.getNScan()) + "]", ctx);       
  ...
  public FileData htmlMenu() throws Exception {
    ...
    if (d.getNScan() > 0) {
      definition = definition + ";print:Печать;del_last:Отменить последнее сканирование";
    }

    HtmlPageMenu p = new HtmlPageMenu("Меню", "Маркировка паллет",
  ...
  public FileData handleMenu(String menu, UserContext ctx) throws Exception {
    if (menu.equals("fin")) {
      ...
      return null;
        ...
        Z_TS_MARK_PALLET_SHKLIST_PRINT f = new Z_TS_MARK_PALLET_SHKLIST_PRINT();        
...
//AM 12.01.2024
class MarkPalletData extends ProcData {
